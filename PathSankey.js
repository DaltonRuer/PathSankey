requirejs.config({
    shim: {
        "extensions/PathSankey/sankeymore": {
            deps: ["extensions/PathSankey/d3.min"],
            exports: 'd3.sankey'
        }
    }
});
        ...
        return {
            ...
            paint: function ($element, layout) {
                ...
                var source = qMatrix.map(function (d) {
                    ...
                    var sep = "";
                    var fullPath = null;
                    // If only one dimension and one measure - the dimension has a comma delimited path
                    if(d.length == 2) {
                        path = d[0].qText;
                        fullPath = {
                            "Path": path,
                        ...
                        for (var i = 0; i < d.length - 1; i++) {
                            path += sep + d[i].qText + '|' + d[i].qElemNumber;
                            sep = ",";
                        }
                        fullPath = {
                            "Path": path,
                    ...
                    // Measure values populate 'Frequency'
                    return fullPath;
                });
                        ...
                        if (tArr.length > 1) {
                            ...
                            $.each(tArr, function (i) {
                                if ($.inArray(this.toString().trim(), sNodes) === -1) {
                                    var sNode = this.toString().trim();
                                    //console.log(sNode)
                                    sNodes.push(sNode);
                                    if(qDim.length == 1)
                                        qElemNumberBySNodes[sNode] = d.qElemNumber;
                                }
                            });
                        }
                    }
                ...
                sNodes.forEach(function (d) {
                    jNodes.push({
                        name: d.toString(),
                        qElemNumber: (qDim.length == 1 ? qElemNumberBySNodes[d.toString()] : null)
                    })
                });
                                ...
                                if ((i + 1) != tArr.length) {
                                    ...
                                    if (tFlag == "no") {
                                        sLinks.push({
                                            ...
                                            "target": cT,
                                            "value": val,
                                            "qElemNumber": (qDim.length == 1 ? qElemNumberBySNodes[this.toString().trim()] : null)
                                        });
                                    }
                                }
                            });
                ...
                var sankey = PathSankey().nodeWidth(15).nodePadding(10).size([width - 10, height - 10]);
                ...
                function linkHoverHtml(d) {
                    ...
                    var html = "<b>" + start + "</b>" + displaySeparateur + "<b>" + end + "</b><br/>" + targetValue
                    return html;
                }

                //Link tooltip
                link.on("mouseover", function (d) {
                    tooltip.html(linkHoverHtml(d));
                ...
                link.on("click", function (d, i) {
                    ...
                    if(qDim.length == 1) {
                        ...
                        _this.backendApi.selectValues(
                            parseInt(d.target.name.split('~')[1].replace('end', qDim.length - 1)), //DAP: As we already selected a link, it makes sense to select the source and target dimensions and filter the data
                ...
                function nodeHoverHtml(d) {
                    var marginX = 0; //Last nodes might not have enough space for the tooltip, so we add a negative margin to push the tooltip on the left
                    ...
                    // Path based graphs only have one dimension and greater than one for multi-dimensional based graph
                    var title = (qDim.length == 1 ? qDim[0] : qDim[level]); // + ' : ' + d.name.split('|')[0];
                    var value = formatNumber(displayFormat, d.value, '', currencySymbol);
                    var html = "<div></div><b>" + title + "</b><br/>" + value + '</div>';
                    return {marginX: marginX, html: html};
                }

                //Node tooltip
                node.on("mouseover", function (d) {
                    var hover = nodeHoverHtml(d);
                    tooltip.html(hover.html);
                    return tooltip.style("visibility", "visible").style("top", (d3.event.pageY + 10) + "px").style("left", (d3.event.pageX + 10 + hover.marginX) + "px");
                })
                .on("mousemove", function (d) {
                    var hover = nodeHoverHtml(d);
                    tooltip.html(hover.html);
                    return tooltip.style("top", (d3.event.pageY + 10) + "px").style("left", (d3.event.pageX + 10 + hover.marginX) + "px");
                })
                .on("mouseout", function () {
                    return tooltip.style("visibility", "hidden");
                });
